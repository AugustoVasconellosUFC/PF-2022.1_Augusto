...
fit (xs, lim) index value = not (exists charVal subStr)
  where
    charVal = dig2char value
    subStr = neib xs index lim
...
set xs index value = xs1 ++ (charVal:xs2)
  where 
    (xs1,_:xs2) = splitAt index xs
    charVal = dig2char value
